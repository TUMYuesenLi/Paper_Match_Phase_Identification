...
for i, test_data in enumerate(data_list):
    ...
    early_stopping = EarlyStopping(patience=10, mode='max'
                                   , save_path=f'aim_TGAN_demo_{test_match_team_list[i]}.pt'
                                   )
    ...
    aim_model.load_state_dict(torch.load(
        f'aim_TGAN_demo_{test_match_team_list[i]}.pt'
    ))
    ...
    early_stopping = EarlyStopping(patience=20, mode='max'
                                   , save_path=f'Invade_TGAN_demo_{test_match_team_list[i]}.pt'
                                   , max_f1_gap=0.2)
    ...
    invade_model.load_state_dict(torch.load(
        f'Invade_TGAN_demo_{test_match_team_list[i]}.pt'
    ))

    Invade_pred_full, Invade_phases_full, Invade_f1_test, Invade_f1_test_each, Invade_event_info \
    ...
    early_stopping = EarlyStopping(patience=20, mode='max', max_f1_gap=0.2
                                   ...
                                   , save_path=f'Scoring_TGAN_demo_{test_match_team_list[i]}.pt'
                                   )
    ...
    Scoring_model.load_state_dict(torch.load(
        f'Scoring_TGAN_demo_{test_match_team_list[i]}.pt'
    ))
    pred_ratios_full, target_ratios_full, Scoring_f1_test, Scoring_f1_test_each, Scoring_event_info \
    ...
    aim_pred, aim, _, _, aim_event_importance_df = test_model(aim_model, match_hetero_dataset, device=device, y_key=0
                               , traditional_features=True
                               # , counter_features=True
                                , return_event_importance=True
                               )
    ...
    Invade_pred, Invade_phases, _, _, invade_event_importance_df = test_model(invade_model, match_hetero_dataset_invade, device=device, y_key=1
                                            , traditional_features=True
                                            , counter_features=True
                                            , return_event_importance=True
                                            )
    ...
    Scoring_pred, Scoring_phases, _, _, scoring_event_importance_df = test_model(Scoring_model, match_hetero_dataset_scoring, device=device, y_key=1
                                              , traditional_features=True
                                                    , return_event_importance=True
                                              )
    ...
    final_data.to_csv(f'Matchphases_{test_match_team_list[i]}_Phase_pred_T-GAN_demo.csv')
...
f1_df.to_csv('Test_f1_per_match_T-GNN.csv', index=False)
mean = f1_df.mean()
std = f1_df.std()
