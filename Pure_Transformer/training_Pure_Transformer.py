...
for i, test_data in enumerate(data_list):
    ...
    early_stopping = EarlyStopping(patience=10, mode='max'
                                   , save_path=f'aim_pure_transformer_demo_{match_team_list[i]}.pt'
                                   )
    ...
    aim_model.load_state_dict(torch.load(
        f'aim_pure_transformer_demo_{match_team_list[i]}.pt'
    ))
    ...
    print('Start training')
    invade_model = SoccerTransformer(
        global_dim=17,
        action_dim=18,
        num_classes=3,
        d_model=128,
    ...
    early_stopping = EarlyStopping(patience=20, mode='max'
                                   , save_path=f'Invade_pure_transformer_demo_{match_team_list[i]}.pt'
                                   , max_f1_gap=0.2)
    ...
    print('Start training')
    Scoring_model = SoccerTransformer(
        global_dim=17,
        action_dim=18,
        num_classes=2,
        d_model=128,
    ...
    early_stopping = EarlyStopping(patience=20, mode='max', max_f1_gap=0.2
                                   ...
                                   , save_path=f'Scoring_pure_transformer_demo_{match_team_list[i]}.pt'
                                   )
    ...
    Scoring_model.load_state_dict(torch.load(
        f'Scoring_pure_transformer_demo_{match_team_list[i]}.pt'
    ))
    ...
    Scoring_pred, Scoring_phases, _, _ = test_model(Scoring_model, match_hetero_dataset_scoring, device=device, y_key=1
                                              )
    ...
    final_data.to_csv(f'Matchphases_{match_team_list[i]}_Phase_pred_pure_transformer_demo.csv')
...
f1_df.to_csv('Test_f1_per_match_Pure_Transformer.csv', index=False)
mean = f1_df.mean()
std = f1_df.std()
